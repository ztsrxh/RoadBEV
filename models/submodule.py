...
def disparity_regression(x, maxdisp, keep_dim):
    assert len(x.shape) == 4
    disp_values = torch.arange(0, maxdisp, dtype=x.dtype, device=x.device)
    disp_values = disp_values.view(1, maxdisp, 1, 1)
    return torch.sum(x * disp_values, 1, keepdim=keep_dim)
...
def build_difference_volume(refimg_fea, targetimg_fea, maxdisp):
    B, C, H, W = refimg_fea.shape
    volume = refimg_fea.new_zeros(B, C, maxdisp, H, W)

    for i in range(maxdisp):
        if i > 0:
            volume[:, :, i, :, i:] = refimg_fea[:, :, :, i:] - targetimg_fea[:, :, :, :-i]
        else:
            volume[:, :, i, :, :] = refimg_fea - targetimg_fea

    volume = volume.contiguous()
    return volume

def build_product_volume(refimg_fea, targetimg_fea, maxdisp):
    B, C, H, W = refimg_fea.shape
    volume = refimg_fea.new_zeros(B, C, maxdisp, H, W)
...
def build_gwc_volume_range(refimg_fea, targetimg_fea, mindisp, maxdisp, num_groups):
    B, C, H, W = refimg_fea.shape
    volume = refimg_fea.new_zeros([B, num_groups, maxdisp-mindisp+1, H, W])
    for i in range(mindisp, maxdisp+1):
        if i > 0:
            volume[:, :, i-mindisp, :, i:] = groupwise_correlation(refimg_fea[:, :, :, i:], targetimg_fea[:, :, :, :-i],
                                                           num_groups)
        else:
            volume[:, :, i, :, :] = groupwise_correlation(refimg_fea, targetimg_fea, num_groups)
    volume = volume.contiguous()
...
def build_gw_diff_volume(refimg_fea, targetimg_fea, maxdisp, num_groups):
    B, C, H, W = refimg_fea.shape
    volume = refimg_fea.new_zeros([B, num_groups, maxdisp, H, W])
    for i in range(maxdisp):
        if i > 0:
...
class BasicBlock(nn.Module):
    def __init__(self, inplanes, planes, stride, downsample, kernel, pad):
        super(BasicBlock, self).__init__()

        self.conv1 = nn.Sequential(convbn(inplanes, planes, kernel, stride, pad, dilation=1),
                                   nn.ReLU())

        self.conv2 = convbn(planes, planes, kernel, 1, pad, dilation=1)
...
class BasicBlock_in(nn.Module):
    def __init__(self, inplanes, planes, stride, downsample, kernel, pad):
        ...
        self.conv1 = nn.Sequential(convin(inplanes, planes, kernel, stride, pad, dilation=1),
                                   nn.LeakyReLU(negative_slope=0.1, inplace=True))

        self.conv2 = convin(planes, planes, kernel, 1, pad, dilation=1)
        self.downsample = downsample
